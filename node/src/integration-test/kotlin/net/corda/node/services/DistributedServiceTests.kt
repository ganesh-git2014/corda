...
class DistributedServiceTests : DriverBasedTest() {
            ...
            notarySpecs = listOf(NotarySpec(clusterName, rpcUsers = listOf(testUser), cluster = ClusterSpec.Raft(clusterSize = clusterSize))))
    {
        ...
        notaryNodes = uncheckedCast(defaultNotaryHandle.nodeHandles.getOrThrow())
        notaryIdentity = defaultNotaryHandle.identity
        aliceProxy = connectRpc(alice)

        assertThat(notaryNodes).hasSize(clusterSize)
        // Make sure each node in the notary cluster has the composite notary identity
        for (notaryNode in notaryNodes) {
            assertThat(notaryNode.nodeInfo.legalIdentities).contains(notaryIdentity)
        }
        // Check that each notary has different identity as a node, plus 1 identity which
        assertThat(notaryNodes.flatMap { it.nodeInfo.legalIdentities - notaryIdentity}.toSet()).hasSameSizeAs(notaryNodes)
